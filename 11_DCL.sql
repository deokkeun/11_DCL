...
CREATE USER kdk_sample IDENTIFIED BY sample1234;
CREATE USER sss_sample IDENTIFIED BY sample123;


-- 2. 새 연결(접속 방법) 추가
-- ORA-01045: 사용자 KDK_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 
-- 로그온이 거절되었습니다

-- 3. 접속 권한 부여 
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO kdk_sample;
...
GRANT CREATE TABLE TO kdk_sample;
ALTER USER kdk_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM; -- 할당
...
GRANT CONNECT, RESOURCE TO kdk_sample;
...
-- kh / kdk_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh) kdk_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO kdk_sample;
...
REVOKE SELECT ON EMPLOYEE FROM kdk_sample;
